...
public class InMemoryStorage implements Storage {
        ...
        public <T> T save(Class<T> entityClass, Long id, T entity) {
            logger.info("Saving entity of type {} with Id: {}", entityClass.getSimpleName(), id);
        ...
        public <T> T findById(Class<T> entityClass, Long id) {
            logger.info("Searching entity of type {} with Id: {}", entityClass.getSimpleName(), id);
            Map<Long, Object> namespace = getNamespace(entityClass);
            T entity=entityClass.cast(namespace.get(id));
            if (entity != null) {
                logger.debug("Found entity: {}", entity);
            } else {
                logger.warn("Entity of type {} with Id: {} not found", entityClass.getSimpleName(), id);
            }
            return entity;
        }
        ...
        public <T> void delete(Class<T> entityClass, Long id) {
            logger.info("Deleting entity of type {} with Id: {}", entityClass.getSimpleName(), id);
            Map<Long, Object> namespace = getNamespace(entityClass);
            if (namespace.remove(id) != null) {
                logger.debug("Entity with Id: {} successfully deleted", id);
            } else {
                logger.warn("Failed to delete. Entity of type {} with Id: {} not found", entityClass.getSimpleName(), id);
            }
        }
        ...
        public <T> Map<Long, T> getAll(Class<T> entityClass) {
            logger.info("Getting all entities of type {}", entityClass.getSimpleName());
            Map<Long, Object> namespace = getNamespace(entityClass);
            Map<Long, T> result = new HashMap<>();
            for (Map.Entry<Long, Object> entry : namespace.entrySet()) {
                result.put(entry.getKey(), entityClass.cast(entry.getValue()));
            }
            logger.info("Got {} entities of type {}", result.size(), entityClass.getSimpleName());
            return result;
        }
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(initFilePath)))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("Trainer")) {
                    ...
                    logger.info("Trainer with Id: {} successfully loaded", id);
                    ...
                    logger.info("Trainee with Id: {} successfully loaded", id);
                    ...
                    logger.info("Training with Id: {} successfully loaded", id);
                } else {
                    throw new IllegalArgumentException("Unknown type: " + parts[0]);
                }
           }
            logger.info("Storage successfully initialized from file: {}\n", initFilePath);
        } catch (IOException|IllegalArgumentException e) {
            logger.error("Failed to initialize storage from file: {}", initFilePath, e);
        }
    }
}
